...
import com.ecommerce.games.dto.request.CheckoutEntityToPaymentDTO;
...
public class GamesCheckoutController {
    ...
    public ResponseEntity<MessageResponseDTO> create (@RequestBody CheckoutGamesRequest request){
        ...
        CheckoutEntityToPaymentDTO paymentDto = criarDto(responseDTO.getCode(), request.getValue());
        service.addEventKafka("SalvaCheckout", paymentDto);
        return ResponseEntity.ok().body(responseDTO);
    }

    private CheckoutEntityToPaymentDTO criarDto(String code, String valor){
        return CheckoutEntityToPaymentDTO.builder()
                .code(code)
                .value(valor)
                .build();
    }
